  ...
  const api: AppContextType["api"] = {
    ...
    getTask: async (taskListId: string, taskId: string) => {
      const response = await axios.get<Task>(
        `/api/listas-de-tareas/${taskListId}/tasks/${taskId}`,
        jsonHeaders
      );
      dispatch({
        type: GET_TASK,
    ...
    deleteTaskList: async (id) => {
      await axios.delete(`/api/listas-de-tareas/${id}`, jsonHeaders);
      dispatch({ type: DELETE_TASKLIST, payload: id });
    },
    fetchTasks: async (taskListId) => {
      const response = await axios.get<Task[]>(
        `/api/listas-de-tareas/${taskListId}/tasks`,
        jsonHeaders
      );
      dispatch({
        type: FETCH_TASKS,
